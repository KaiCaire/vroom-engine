...
bool Render::Awake()
{
	...
	SDL_Window* window = Application::GetInstance().window.get()->window;
	...
	{
		camera.w = Application::GetInstance().window.get()->width * scale;
		camera.h = Application::GetInstance().window.get()->height * scale;
		camera.x = 0;
		camera.y = 0;
	}
